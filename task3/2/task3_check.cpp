...
int main(int argc, char *argv[]) {
    ...
    int check = 0;
    if((std::strcmp(argv[1], "Sinus.txt") != 0 && std::strcmp(argv[1], "Square.txt") != 0 && std::strcmp(argv[1], "Power.txt") != 0)||
        argc != 2 || std::strcmp(argv[1], "help") == 0){
        std::cerr << "Введите файл с разрешением .txt (Sinus.txt Square.txt Power.txt), который вы хотите проверить";
    }
    else{
        if(std::strcmp(argv[1], "Sinus.txt") == 0){
            int err = Sinus(argv[1]);
